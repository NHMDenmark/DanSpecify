...
CREATE OR REPLACE DEFINER=`root`@`localhost` PROCEDURE `AddAttachmentWithFileNameToAltCatNr`(
...
SET @coid := (
    ...
    WHERE AltCatalogNumber = @altcatnr
    LIMIT 1
);

IF @coid IS NOT NULL THEN 
-- Alt Catalog Number found; Proceed with insert. 
